...
This doc defines the convention for this app's JavaScript — decided once rather than improvised page by page. It started (Phase 4) with no JS at all in the app; that's no longer true (see "Status" below for what actually exists now), but the shape decided here has held up unchanged as real features were added.
...
- **One script tag, forever.** `main.js` loads on every page. As new blueprints or features get added, they just get a new `import` line in `main.js` — no per-template `{% block scripts %}` bookkeeping to maintain. One deliberate exception: `base.html`'s dark-mode anti-flash script (reads `localStorage`/`prefers-color-scheme` and toggles the `dark` class before first paint) is an inline, synchronous `<script>` in `<head>`, not a `main.js` import — it has to run and finish before the stylesheet is even requested, which a `type="module"` script loaded at the end of `<body>` can't do. It's the one page-chrome behavior that isn't in `main.js`, and it's inline specifically because of that timing constraint, not because it was forgotten.
...
`main.js` initializes the navbar's user-menu dropdown (`base.html`'s `[data-user-menu-toggle]`/`[data-user-menu]`), every `[data-inline-edit]` widget on the page (now that more than one blueprint uses it), a site-wide page-navigation loading bar (`base.html`'s `#page-loading-bar`, shown on any same-origin link click or form submit, to give feedback on slower deployed-server round trips), a generic confirm-before-submit hook for destructive forms (any `<form data-confirm="...">` gets a `window.confirm()` gate before it's allowed to submit — used by the book-delete and invite-revoke/rating-delete buttons), and the dark-mode toggle button's click handler (`base.html`'s `[data-theme-toggle]` — flips the `dark` class and persists the choice to `localStorage`; the *initial* theme on page load is set by the separate inline anti-flash script in `<head>`, see "Why this shape" above).

`books.js` has real behavior of its own: the book list's filter dropdown (`[data-filter-toggle]`/`[data-filter-menu]`); the rating score picker (`[data-star-picker]`) — a directly tappable/draggable star widget layered visually over a `sr-only` (but still focusable/keyboard-accessible) `<select>`, which stays the actual form control and source of truth; and the Open Library auto-fill widget's own in-place spinners for its two `fetch()` calls (deliberately separate from the page-navigation bar — see `CLAUDE.md`'s "Open Library auto-fill" note). `admin.js` handles the Safari date-input Clear button (`[data-clear-target]`) and copy-to-clipboard buttons (`[data-copy-value]`) on the invites/members pages. `auth.js` wires up the login page's two inline token-entry forms (`#invite-token-form`/`#reset-token-form`), redirecting to the existing `register(token)`/`reset_password(token)` routes for validation. Further features drop into the relevant blueprint file when they come up.
