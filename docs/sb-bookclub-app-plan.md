...
**Books**
- Add a book: title, author, cover image (manual URL or auto-fetched), genre/tags, date added, who added it
- Edit/delete a book
...
**Views**
- Book list: sortable/filterable by status, average rating, date, genre
...
4. Rating scale: 1–5, 1–10, half-star increments? Decide early — it affects DB schema and UI.
...
| Frontend | **Jinja2 templates** + Tailwind CSS (CDN) or plain CSS | Server-rendered, no separate frontend framework needed |
...
- **Book**: id, title, author, cover_url, genre, date_added, added_by (FK → Member), status
...
- [ ] Base Jinja template + Tailwind/CSS setup
...
- [ ] Stats page (who rates highest/lowest, most active reviewer, genre breakdown)
